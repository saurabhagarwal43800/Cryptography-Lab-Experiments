...
class Columnar{
      ...
      System.out.print("\nEnter plaintext(enter in lower case without spaces): ");
      String message = sc.next();
      System.out.print("\nEnter key in numbers(without spaces): ");
      String key = sc.next();

      /* columnCount would store number of columns */
      int columnCount = key.length();

      /* rowCount will store number of rows*/
 ...
 int currentCol= ( (int)key.charAt(i) - 48 ) -1;
          ...
          System.out.print("Cipher Array(read column wise): \n");
          for(i=0;i<rowCount;i++){
                for(j=0;j<columnCount;j++){
     ...
     static int[][] decrypt(int plainText[][], int cipherText[][], String message, int rowCount, int                                                                                                          columnCount, String key){
            ...
            System.out.print("Plain Array(read row wise): \n");
            for(i=0;i<rowCount;i++){
                 for(j=0;j<columnCount;j++){
